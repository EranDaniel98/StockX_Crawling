...
def main():
    ...
    if(input('Is is first run? Y/N ').lower() == 'y'):
    # First time run:
        pred = predict()
        ...
        if os.path.isfile('./proccessed_dataframe.csv') == False:
        ...
        # Every other run:
        pred = predict('./csv_files/proccessed_dataframe.csv')
        sub_pred = sub_predict(pred)
        sub_pred.predict_shoe_price('https://stockx.com/adidas-yeezy-foam-rnnr-ochre')
